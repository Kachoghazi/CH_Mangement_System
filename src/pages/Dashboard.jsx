...
const Dashboard = ({
    ...
    const syncStudents = () => {
      try {
        ...
        if (
          ...
          JSON.stringify(storedStudents) !== JSON.stringify(dashboardStudents)
        ) {
  ...
  const extractMonthYear = useCallback(
    (admissionMonth) => {
    ...
    (event) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          // Helper function to convert Excel date (serial number or Date object) to YYYY-MM-DD string
          const parseExcelDate = (value) => {
            ...
            if (typeof value === 'string') {
              ...
              if (mdyMatch) {
                // Assume it's DD/MM/YYYY since this is for Pakistan
                const [, day, month, year] = mdyMatch;
                return `${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`;
              }
            ...
            if (typeof value === 'number') {
              // Excel's epoch is January 1, 1900 (but with a bug where it thinks 1900 was a leap year)
              // So we need to subtract 1 if the date is after Feb 28, 1900 (serial number > 59)
          ...
          const importedStudents = jsonData.map((row, index) => {
            ...
            const studentId =
              row['Student ID'] || row['studentId'] || row['ID'] || `IMP-${Date.now()}-${index}`;
            ...
            // Parse admission date properly (handles Excel serial numbers, Date objects, and various string formats)
            const rawAdmissionDate =
              ...
              row['DOA'] ||
              row['doa'];
            const admissionDate = parseExcelDate(rawAdmissionDate);
            ...
            const paid = parseFloat(
              row['Paid'] || row['paid'] || row['Total Paid'] || row['tuitionPaid'] || 0,
            );
            ...
            return {
              ...
              admissionDate,
              admissionMonth,
              status: ['Active', 'Inactive', 'Left', 'Passout'].includes(status)
                ? status
                : 'Active',
              gender,
          ...
          if (
            window.confirm(
              `Import ${importedStudents.length} students from Excel? This will add them to your existing students.`,
            )
          ) {
            ...
            alert(
              `Successfully imported ${newStudents.length} students. ${duplicates > 0 ? `${duplicates} duplicates were skipped.` : ''}`,
            );
  ...
  const filteredStudents = useMemo(() => {
    ...
    if (filterStatus !== 'all') {
      filtered = filtered.filter((student) => (student.status || 'Active') === filterStatus);
    }
  ...
  const stats = useMemo(() => {
    ...
    const activeStudents = dashboardStudents.filter(
      (s) => (s.status || 'Active') === 'Active',
    ).length;
    const inactiveStudents = dashboardStudents.filter(
      (s) => (s.status || 'Active') === 'Inactive',
    ).length;
    const leftStudents = dashboardStudents.filter((s) => (s.status || 'Active') === 'Left').length;
    const passoutStudents = dashboardStudents.filter(
      (s) => (s.status || 'Active') === 'Passout',
    ).length;
    ...
    const currentMonthActive = currentMonthStudents.filter(
      (s) => (s.status || 'Active') === 'Active',
    ).length;
    const currentMonthInactive = currentMonthStudents.filter(
      (s) => (s.status || 'Active') === 'Inactive',
    ).length;
    const currentMonthLeft = currentMonthStudents.filter(
      (s) => (s.status || 'Active') === 'Left',
    ).length;
    const currentMonthPassout = currentMonthStudents.filter(
      (s) => (s.status || 'Active') === 'Passout',
    ).length;
  ...
  const getFeesForMonth = useCallback(
    (month, year) => {
      if (month === 'all') {
  ...
  const removeCustomYear = useCallback(
    (yearToRemove) => {
          ...
          >
            <div>
              ...
              <p style={{ margin: '5px 0 0 0', opacity: 0.9, fontSize: '14px' }}>
                Complete info about {viewingStudent.name}
              </p>
            </div>
            <button
        ...
        >
          ...
          >
            <div>
              ...
              <p style={{ margin: '5px 0 0 0', opacity: 0.9, fontSize: '14px' }}>
                Add or remove years from the filter list
              </p>
            </div>
            <button
                ...
                >
                  <span
                    style={{
                      fontWeight: '600',
                      color:
                        year === 'all' ? '#3b82f6' : year === currentYear ? '#10b981' : '#1e293b',
                      fontSize: '16px',
                    }}
                  >
                    {year === 'all'
  ...
  return (
    <div
      className="dashboard"
      style={{ minHeight: '100vh', background: '#f8fafc', padding: '15px' }}
    >
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '15px', flex: 1 }}>
            ...
            {academyConfig.showLogo !== false && academyConfig.logo && (
              ...
              >
                {academyConfig.logoType === 'image' &&
                academyConfig.logo.startsWith('data:image') ? (
                  <img
          ...
          >
            ...
            >
              {academyConfig.fees?.currencySymbol || '₹'}
              {stats.allTimeTotalPaid.toLocaleString()}
            </div>
                ...
                >
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ color: '#1e293b', fontWeight: '700' }}>
                      {formatCurrency(selectedMonthFees.total)}
                    </div>
                  </div>
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ color: '#ef4444', fontWeight: '600' }}>Due</div>
                    <div style={{ color: '#ef4444', fontWeight: '700' }}>
                      {formatCurrency(selectedMonthFees.due)}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <select
                  ...
                  style={{
                    ...
                    transition: 'border 0.2s ease',
                    borderColor:
                      filterStatus !== 'all' ? getStatusInfo(filterStatus).color : '#e2e8f0',
                  }}
                >
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ position: 'relative' }}>
                  <input
                    ...
                    onBlur={(e) =>
                      (e.target.style.borderColor = searchQuery ? '#f59e0b' : '#e2e8f0')
                    }
                  />
                  <Search
                    size={16}
      ...
      >
        ...
        {filteredStudents.length > 0 ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '1100px' }}>
              ...
              <tbody>
                {filteredStudents.map((student, index) => {
                  ...
                  const { month: admissionMonth, year: admissionYear } = extractMonthYear(
                        ...
                        {editingStatus === student.studentId ? (
                          <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                            ...
                            >
                              ...
                              <option value="Left" style={{ color: '#f59e0b', fontWeight: '600' }}>
                                Left
                              </option>
                              <option
                                value="Passout"
          ...
          >
            ...
            <p style={{ margin: '0 auto 20px', fontSize: '13px', maxWidth: '400px' }}>
