...
import { 
  Plus, Minus, Calculator, AlertCircle, Calendar,
  Percent, Tag, BadgePercent, ArrowRight
} from "lucide-react";
...
const NewAdmission = ({ addStudent, editingStudent = null }) => {
  ...
  const monthlyFee = selectedCourseFeeData ? Number(selectedCourseFeeData.amount) : (selectedCourse ? Number(selectedCourse.fees) : 0);
  ...
  const generateInstallmentMonths = () => {
    ...
    const { month: startMonth, year: startYear } = calculateInstallmentStartMonth(formData.admissionDate);
  ...
  useEffect(() => {
    if (editingStudent && courses.length > 0) {
      ...
      if (editingStudent.course) {
        ...
        if (course) {
          const courseFeeData = courseFeesData.find(fee => fee.courseId.toString() === course.id.toString());
  ...
  const handleChange = (e) => {
    ...
    if (name === "course") {
      ...
      const isDuplicate = existingStudentIds.includes(studentId) && 
  ...
  }, [formData.installmentPlan, formData.numberOfInstallments, formData.admissionDate, formData.feePaid, totalCourseFees]);
  ...
  const validateForm = () => {
    ...
    if (formData.course && selectedCourse && !selectedCourseFeeData) {
      newErrors.course = "This course doesn't have a fee set in Fees Management. Please set fee first.";
    }
  ...
  const handleSubmit = (e) => {
    ...
    const studentData = {
      ...
      selectedExtraFees,
      installmentMonths: formData.installmentPlan === "installment" ? formData.numberOfInstallments : 1,
      admissionDate: formData.admissionDate || todayDate,
      installmentMonthsList: formData.installmentPlan === "installment" ? installmentMonthsList : [],
      ...
      status: "active",
      paymentStatus: feeDue === 0 ? "fully_paid" : 
                    formData.feePaid === 0 ? "unpaid" : "partial"
    };
    ...
    try {
      ...
      if (formData.installmentPlan === "installment" && installmentMonthsList.length > 0) {
        const installmentMonthsStr = installmentMonthsList.map(m => `${m.month} ${m.year}`).join(", ");
  ...
  // Styles
  const styles = {
    container: { padding: "20px", background: "#f8fafc", minHeight: "100vh", fontFamily: "'Segoe UI', sans-serif" },
    card: { background: "white", padding: "25px", borderRadius: "15px", boxShadow: "0 5px 15px rgba(0,0,0,0.1)", maxWidth: "1000px", margin: "0 auto" },
    header: { fontSize: "24px", fontWeight: "700", color: "#1e293b", marginBottom: "20px", paddingBottom: "10px", borderBottom: "2px solid #e2e8f0" },
    formRow: { display: "flex", gap: "15px", marginBottom: "20px", flexWrap: "wrap" },
    formGroup: { flex: "1", minWidth: "200px" },
    label: { display: "block", marginBottom: "8px", fontWeight: "600", color: "#475569", fontSize: "13px" },
    requiredLabel: { display: "block", marginBottom: "8px", fontWeight: "600", color: "#dc2626", fontSize: "13px" },
    input: { width: "100%", padding: "12px 15px", border: "1px solid #e2e8f0", borderRadius: "8px", fontSize: "14px", background: "#f8fafc" },
    errorInput: { borderColor: "#dc2626", background: "#fef2f2" },
    select: { width: "100%", padding: "12px 15px", border: "1px solid #e2e8f0", borderRadius: "8px", fontSize: "14px", background: "#f8fafc", cursor: "pointer" },
    textarea: { width: "100%", padding: "12px 15px", border: "1px solid #e2e8f0", borderRadius: "8px", fontSize: "14px", background: "#f8fafc", minHeight: "100px", fontFamily: "inherit" },
    errorMessage: { color: "#dc2626", fontSize: "12px", marginTop: "5px", display: "flex", alignItems: "center", gap: "5px" },
    feeTypeSection: { marginTop: "25px", marginBottom: "25px", padding: "20px", background: "#f8fafc", borderRadius: "12px", border: "1px solid #cbd5e1" },
    feeTypeHeader: { fontSize: "18px", fontWeight: "700", color: "#1e293b", marginBottom: "15px", display: "flex", alignItems: "center", gap: "10px" },
    extraFeeGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: "10px", marginTop: "15px" },
    extraFeeCard: { 
      padding: "15px", 
      borderRadius: "8px", 
    ...
    summaryCard: { background: "#1e293b", padding: "20px", borderRadius: "12px", marginTop: "15px", color: "white" },
    summaryGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: "15px", marginTop: "10px" },
    summaryItem: { textAlign: "center", padding: "12px", borderRadius: "8px", background: "rgba(255,255,255,0.1)" },
    summaryLabel: { fontSize: "12px", color: "#cbd5e1", marginBottom: "5px" },
    summaryValue: { fontSize: "20px", fontWeight: "700", color: "white" },
    paymentSection: { background: "#f1f5f9", padding: "20px", borderRadius: "12px", marginTop: "15px", border: "1px solid #e2e8f0" },
    installmentSection: { background: "#e0f2fe", padding: "15px", borderRadius: "10px", marginTop: "15px", border: "1px dashed #0ea5e9" },
    installmentNote: { fontSize: "12px", color: "#0369a1", marginTop: "10px", padding: "8px", background: "rgba(14, 165, 233, 0.1)", borderRadius: "6px" },
    buttonGroup: { display: "flex", gap: "10px", marginTop: "20px", flexWrap: "wrap" },
    submitButton: { background: "#3b82f6", color: "white", border: "none", padding: "14px 30px", borderRadius: "8px", fontSize: "15px", fontWeight: "600", cursor: "pointer", display: "flex", alignItems: "center", gap: "8px" },
    resetButton: { background: "#f1f5f9", color: "#475569", border: "1px solid #e2e8f0", padding: "14px 30px", borderRadius: "8px", fontSize: "15px", fontWeight: "600", cursor: "pointer", display: "flex", alignItems: "center", gap: "8px" },
    admissionMonthSection: { background: "#f0f9ff", padding: "15px", borderRadius: "8px", marginBottom: "15px", border: "1px solid #bae6fd" },
    admissionMonthHeader: { display: "flex", alignItems: "center", gap: "8px", marginBottom: "10px" },
    courseInfo: { background: "#e0f2fe", padding: "15px", borderRadius: "8px", marginTop: "10px", borderLeft: "4px solid #0ea5e9" },
    installmentSchedule: { background: "#f0fdf4", padding: "15px", borderRadius: "8px", marginTop: "15px", border: "1px solid #bbf7d0" },
  ...
  return (
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        <div style={{ background: "#e0f2fe", padding: "10px", borderRadius: "6px", marginBottom: "15px", border: "1px solid #7dd3fc", fontSize: "13px", color: "#0369a1" }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={styles.formRow}>
            <div style={styles.formGroup}>
              ...
              >
                ...
                {courses.map(c => {
                  const courseFee = courseFeesData.find(fee => fee.courseId.toString() === c.id.toString());
                  const totalFee = courseFee ? (Number(courseFee.amount) * Number(c.months)) : 0;
                  
                  return (
                    <option key={c.id} value={c.title}>
                      {c.title} ({c.months} months) - {courseFee ? `PKR${courseFee.amount}/month (Total: PKR${totalFee})` : 'Fee not set'}
                    </option>
                  );
                })}
              </select>
              ...
              {selectedCourse && (
                <div style={styles.courseInfo}>
                  ...
                  <div><strong>Duration:</strong> {courseDuration} months</div>
                  
                  {selectedCourseFeeData ? (
                    <>
                      <div><strong>Monthly Fee:</strong> PKR{monthlyFee.toLocaleString()}/month</div>
                      <div style={{ 
                        background: "#d1fae5", 
                        padding: "8px", 
                        borderRadius: "6px", 
                        ...
                        <div style={{ 
                          ...
                          <span style={{ fontWeight: "bold", color: "#065f46" }}>Total Course Fees:</span>
                          <span style={{ 
                            fontWeight: "bold", 
                            color: "#065f46",
                            fontSize: "18px"
                    ...
                    <div style={styles.warningBox}>
                      ...
                      <div style={{ fontSize: "12px", color: "#92400e" }}>
                        This course doesn't have a fee set in Fees Management. 
                        Please set the fee in Fees Management page first.
                      </div>
                    </div>
                  )}
          ...
          <div style={styles.feeTypeSection}>
            <h3 style={styles.feeTypeHeader}>
              💰 Extra Fees (Optional)
            </h3>
            
            <p style={{ fontSize: "14px", color: "#475569", marginBottom: "15px" }}>
              Select any extra fees that apply. These will be added to the total fee and must be paid separately from installment payments.
            </p>
                ...
                <div style={{ 
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: '#b45309' }}>
                        {discountType === 'percentage' ? `${discountValue}%` : `PKR ${discountValue}`}
                        {discountReason && ` - ${discountReason}`}
                      </div>
                    </div>
                  </div>
            ...
            <div style={styles.summaryCard}>
              ...
              <div style={styles.summaryGrid}>
                ...
                <div style={styles.summaryItem}>
                  ...
                  <div style={{...styles.summaryValue, color: formData.feePaid > 0 ? "#10b981" : "white"}}>
                  ...
                  <>
                    <div style={{ marginTop: "15px" }}>
                      ...
                      <select 
                        ...
                        onChange={handleChange} 
                        style={{...styles.select, ...(errors.numberOfInstallments && styles.errorInput)}}
                      >
                    ...
                    <div style={{ 
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
                        <span style={{ fontSize: "14px", color: "#475569" }}>Total Course Fee:</span>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
                        <span style={{ fontSize: "14px", color: "#475569" }}>Remaining Course Fee:</span>
                        <span style={{ fontWeight: "600", color: "#1e293b" }}>
                          PKR{Math.max(0, totalCourseFees - Math.min(formData.feePaid, totalCourseFees)).toLocaleString()}
                        </span>
                      </div>
                      <div style={{ 
                        display: "flex", 
                        ...
                        <span style={{ fontSize: "14px", color: "#475569", fontWeight: "bold" }}>Installment Amount:</span>
                    ...
                    <div style={styles.installmentNote}>
                      ...
                      <ul style={{ margin: "5px 0 0 0", paddingLeft: "15px" }}>
                        <li>Installments apply only to Course Fee (PKR{totalCourseFees.toLocaleString()})</li>
                        <li>Extra Fees (PKR{extraFeesTotal.toLocaleString()}) must be paid in full at admission</li>
                        <li>If admission date is after 20th of month, installments start from next month</li>
                        <li>Monthly Fee: PKR{monthlyFee.toLocaleString()}/month × {courseDuration} months = PKR{totalCourseFees.toLocaleString()}</li>
                      </ul>
                    </div>
                  </>
                )}
            ...
            <div style={styles.formGroup}>
              ...
              <div style={{ display: "flex", gap: "20px", marginTop: "8px" }}>
                {["male", "female", "other"].map(g => (
                  <label key={g} style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", fontSize: "14px", color: "#475569" }}>
                    <input 
                      type="radio" 
                      name="gender" 
                      value={g} 
          ...
          {selectedCourse && !selectedCourseFeeData && (
            <div style={{ 
              ...
              <p style={{ margin: "0 0 10px 0", color: "#991b1b" }}>
                The selected course "{selectedCourse.title}" doesn't have a fee set in Fees Management.
              </p>
              <p style={{ margin: "0", color: "#991b1b" }}>
                Please go to <strong>Fees Management</strong> page and set the fee for this course first.
              </p>
            </div>
          )}
        </form>
